...
function initAuthForms() {
    ...
    const canToggleForms = Boolean(loginForm && registerForm);

    const setActiveForm = (target) => {
        if (!canToggleForms) return;
    ...
    switchFormLinks.forEach(link => {
        link.addEventListener('click', (e) => {
            const targetForm = (link.getAttribute('data-form') || 'login').toLowerCase();
            ...
            e.preventDefault();
            setActiveForm(targetForm === 'register' ? 'register' : 'login');
        });
    });

    if (canToggleForms) {
        const url = new URL(window.location.href);
        const formParam = (url.searchParams.get('form') || '').toLowerCase();
        const hashTarget = url.hash.replace('#', '').toLowerCase();
        const initialTarget = formParam || hashTarget;

        if (initialTarget === 'register') {
    ...
    const togglePasswordButtons = document.querySelectorAll('button.toggle-password');
